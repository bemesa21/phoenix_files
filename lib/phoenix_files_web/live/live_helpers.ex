defmodule PhoenixFilesWeb.LiveHelpers do
  ...
  def loader(assigns) do
    assigns
    |> assign(assigns_to_attributes(assigns, [:id, :class]))
  ...
  defp show_loader(js \\ %JS{}, id) do
    JS.show(js, to: "##{id}", transition: {"ease-in duration-300", "opacity-0", "opacity-100"})
  end

  defp hide_loader(js \\ %JS{}, id) do
    JS.hide(js, to: "##{id}", transition: {"ease-out duration-300", "opacity-100", "opacity-0"})
  end
